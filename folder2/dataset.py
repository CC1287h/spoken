...
def collate_fn(batch):
    ...
    noisy_real = torch.stack([pad(x["noisy_real"]) for x in batch])
    noisy_imag = torch.stack([pad(x["noisy_imag"]) for x in batch])
    clean_real = torch.stack([pad(x["clean_real"]) for x in batch])
    clean_imag = torch.stack([pad(x["clean_imag"]) for x in batch])

    noisy = torch.stack([noisy_real, noisy_imag], dim=1)
    clean = torch.stack([clean_real, clean_imag], dim=1)

    mask = torch.zeros(len(batch), 1, 1, max_len)
    for i, l in enumerate(spec_lengths):
        mask[i, :, :, :l] = 1.0

    return noisy, clean, mask, wave_lengths
